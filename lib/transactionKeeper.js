...
class TransactionKeeper {
    ...
    // get secure random uint256. Return as regular JS number
    _getRandom() {
        // XXX: I'm certain this isn't the best way to do this lol
        //      I bet there are timing side channels from CPU optimizing arithmetic too haha
        var rand = 0;
        var randBytes = randomBytes(32);
        for (var i = 0; i < 32; i++)
            rand = rand * 256 + randBytes[i];
        return rand;

        //return Math.floor(Math.random() * 2**256); // NOTE: obviously not secure RNG, just for testing
    }
